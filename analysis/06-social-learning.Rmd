...
df_group_success <- targets::tar_read(success)

df_group_success <- df_group_success %>% 
  filter(.step. >= sharing.start,
         learning.mechanism == "learn-socially") %>% 
  select(run = .run.number., step = .step., pubs.vs.data, rdm.cost, 
         agent.orientation, contains("grants")) %>% 
  mutate(across(contains("grants"), as.numeric))
```



# Number of sharing agents
```{r data-sharers-overview-myopic, fig.height=5.5, fig.width=7}
df_clean %>% 
...
```{r data-sharers-overview-long-term, fig.height=5.5, fig.width=7}
df_clean %>% 
...
```{r data-sharers-smooth-myopic, fig.height=5.5, fig.width=7}
df_clean %>% 
  filter(agent.orientation == "all-myopic") %>% 
  clean_pub_weight() %>% 
...
```{r data-sharers-smooth-longterm, fig.height=5.5, fig.width=7}
df_clean %>% 
  filter(agent.orientation == "all-long-term") %>% 
  clean_pub_weight() %>% 
  ggplot(aes(step, count.groups.with..data.sharing.., 
...
```{r data-sharers-smooth-combined, fig.height=5.5, fig.width=7}
df_clean %>% 
  ggplot(aes(step, count.groups.with..data.sharing.., 
...
# Datasets shared
```{r data-sets-smooth-myopic, fig.height=5.5, fig.width=7}
df_clean %>% 
  filter(agent.orientation == "all-myopic") %>% 
  ...
  labs(y = "# of available datasets",
       ...
       x = "Time")
```
```{r data-sets-smooth-longterm, fig.height=5.5, fig.width=7}
df_clean %>% 
  filter(agent.orientation == "all-long-term") %>% 
  ggplot(aes(step, count.datasets, 
             colour = factor(rdm.cost))) +
  ...
  labs(y = "# of available datasets",
       ...
       x = "Time")
```
```{r data-sets-smooth-combined, fig.height=5.5, fig.width=7}
df_clean %>% 
  ggplot(aes(step, count.datasets, 
             colour = factor(agent.orientation))) +
  geom_smooth() +
...
```{r publications-smooth-combined, fig.height=5.5, fig.width=7}
df_clean %>% 
...
```{r publications-smooth-combined-end, fig.height=6, fig.width=10}
df_clean %>% 
  filter(step == 500) %>% 
  clean_pub_weight() %>% 
...
```{r ginis-myopics, fig.height=7, fig.width=7}
pdata %>% 
  filter(agent.orientation == "all-myopic") %>% 
...
```{r ginis-longterms, fig.height=7, fig.width=7}
pdata %>% 
  filter(agent.orientation == "all-long-term") %>% 
  ggplot(aes(step, value, colour = factor(rdm.cost))) +
  geom_smooth() +
...
```{r shared-data-by-quantile-long-term, fig.height=5.5, fig.width=7}
df_data_means %>% 
  display_quantity(mean_datasets) +
  labs(y = "# of datasets")
```

```{r shared-data-by-quantile-myopic, fig.height=5.5, fig.width=7}
df_data_means %>% 
...
```{r sharers-by-quantile-long-term, fig.height=5.5, fig.width=7}
df_data_sharers %>% 
  display_quantity(share_of_sharers) +
  labs(y = "% of groups currently sharing data")
```

```{r sharers-by-quantile-myopic, fig.height=5.5, fig.width=7}
df_data_sharers %>% 
...
```{r grants-by-quantile-long-term, fig.height=6.5, fig.width=6.5}
pdata <- df_grants %>% 
