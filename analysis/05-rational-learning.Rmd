...
df_group_success <- df_group_success %>% 
  filter(.step. >= sharing.start,
         learning.mechanism == "learn-rationally") %>% 
...
```{r datasets-smooth-combined-emnd, fig.height=5, fig.width=8.5}
df_clean %>% 
  filter(step == 500) %>% 
  clean_pub_weight() %>% 
...
```{r ginis-myopics, fig.width=7, fig.height=7}
pdata %>% 
  filter(agent.orientation == "all-myopic") %>% 
...
```{r ginis-longterms, fig.width=7, fig.height=7}
pdata %>% 
  filter(agent.orientation == "all-long-term") %>% 
  ggplot(aes(step, value, colour = factor(rdm.cost))) +
  geom_smooth() +
